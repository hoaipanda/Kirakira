...
public abstract class MagicBaseView extends GLSurfaceView implements GLSurfaceView.Renderer {
    ...
    protected void onFilterChanged() {
        if (filter != null) {
            filter.onInputSizeChanged(surfaceWidth, surfaceHeight);
            filter.onInputSizeChanged(imageWidth, imageHeight);
        }
    }

    public void setFilter(final MagicFilterType type, final Context appctx) {
        queueEvent(new Runnable() {
            ...
            public void run() {
                if (filter != null)
                    filter.destroy();
                filter = null;
                filter = MagicFilterFactory.initFilters(type, appctx);
                if (filter != null)
                    filter.init();
                onFilterChanged();
            }
    ...
    public void setFilter(final GPUImageFilter filter1) {
        queueEvent(new Runnable() {
            @Override
            public void run() {
                filter = filter1;
                if (filter != null) {
                    Log.d("datdb", "filter khac null");
                    filter.init();
                }
                onFilterChanged();
            }
    ...
    public abstract void savePicture(GetPictureTask getPictureTask);
